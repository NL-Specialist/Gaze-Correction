...
class CameraModule:
    ...
    def capture_frame_from_queue(self, image_dir):
        ...
        left_eye_frame = self.eyes_processor.get_left_eye_region(frame)
        ...
        right_eye_frame = self.eyes_processor.get_right_eye_region(frame)
    ...
    def get_frame(self, stream):
        try:
            ...
            if stream == "live-video-left":
                ...
                # Frame with rectangles over the eyes
                processed_frame = self.eyes_processor.draw_selected_landmarks(frame, show_eyes=True, show_mouth=False, show_face_outline=False, show_text=False) # self.eyes_processor.process_frame(frame, draw_rectangles=True)
                ret, buffer = cv2.imencode('.jpg', processed_frame)
                if ret:
                    return buffer.tobytes()
                logging.error("Failed to encode processed frame to JPEG")
